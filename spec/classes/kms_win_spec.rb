  ...
  on_supported_os(facterversion: '2.4').each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'passes with minimal params' do
        let(:params) { { key_management_service_name: 'kms.contoso.com' } }

        it {
          is_expected.to contain_registry_value('KeyManagementServiceName').with(
            'ensure' => 'present',
        ...
        it {
          is_expected.to contain_registry_value('KeyManagementServicePort').with(
            'ensure' => 'present',
        ...
        it {
          is_expected.to contain_exec('slmgr_activation').that_subscribes_to(
            [
        ...
        it {
          is_expected.to contain_exec('slmgr_activation').with(
      ...
      context 'passes with additional params' do
        let(:params) { { key_management_service_name: 'kms.contoso.com', key_management_service_port: '443' } }
